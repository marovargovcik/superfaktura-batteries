...
import superfaktura.{CliError, Money, OcrResult, ReceiptBytes, ReceiptMedia}
...
class ClaudeOcrTest extends AnyFreeSpec with Matchers:
  ...
  private def algebra(client: Client[IO]): superfaktura.OcrAlgebra[IO] =
  ...
  "read" - {
    ...
    "posts a base64 image block and forces the record_receipt tool" in {
      ...
      algebra(capturing(captured, toolUse("""{"amount":1.0,"currency":"EUR","date":"2026-06-13"}""")))
        ...
        .unsafeRunSync()
      val request = captured.get.unsafeRunSync().hcursor
    ...
    "sends a PDF as a document block" in {
      ...
      algebra(capturing(captured, toolUse("""{"amount":1.0,"currency":"EUR","date":"2026-06-13"}""")))
        ...
        .unsafeRunSync()
      val source = captured.get.unsafeRunSync().hcursor.downField("messages").downN(0).downField("content").downN(0)
    ...
    "sends the x-api-key and anthropic-version headers" in {
      ...
      algebra(clientOf(app)).read(bytes, ReceiptMedia.Jpeg).unsafeRunSync().amount shouldBe
        Some(Money(BigDecimal("1.0"), "EUR"))
    }
  }
end ClaudeOcrTest
