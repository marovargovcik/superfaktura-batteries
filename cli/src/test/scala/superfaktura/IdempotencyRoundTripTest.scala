  ...
  "running plan/apply repeatedly across csv, then +receipts, then +rules converges" in {
    ...
    val test =
      for
        ...
        nextId <- Ref.of[IO, Long](1L)
        creates <- Ref.of[IO, Int](0)
        ...
        cycle = (withReceipts: Boolean) =>
          PlanProgram.run[IO](csvPath, Option.when(withReceipts)(receipts).flatten) *> ApplyProgram.run[IO]
        ...
        finalStore <- store.get
      yield
        totalCreates shouldBe 1
        finalStore should have size 1
    test.unsafeRunSync()
  }
